...
sint8 ICACHE_FLASH_ATTR espconn_esp_sent(struct espconn *pconn, uint8_t *buf, uint16_t len,uint8 force)
{
    sint8 res;
...
user_esp_platform_get_info(struct espconn *pconn, uint8 *pbuffer)
{
    ...
    if (pbuf != NULL) {
        ...
        espconn_esp_sent(pconn, pbuf, os_strlen(pbuf),0);
    }
}
...
#endif
        espconn_esp_sent(pespconn, pbuf, os_strlen(pbuf),0);
    }
    //os_timer_arm(&beacon_timer, BEACON_TIME, 0);//
}
...
user_platform_rpc_battery_rsp(struct espconn *pespconn, char* data_buf,int data_buf_len, int nonce, int status)
{
    ...
    #endif
    espconn_esp_sent(pespconn, data_buf, os_strlen(data_buf),0);
...
#if 0
#define ESP_SEND_SMS_FRAME "{\"path\": \"/v1/resource/sms/\", \"method\": \"GET\", \"get\": {\"action\": \"hello_esp8266\", \"phone\": \"13761358055\"}}\n"
#define SMS_MSG_LEN 200
void ICACHE_FLASH_ATTR 
    user_platform_send_sms()
{
    char* sms_msg = (char*)os_zalloc(SMS_MSG_LEN);
    os_sprintf(sms_msg,ESP_SEND_SMS_FRAME);

    if(user_conn.state == ESPCONN_NONE){
        os_printf("conn state error...\r\n");
        return;
    }
    ...
    if(g_mac){
        ESP_DBG("G_MAC: %s\r\n",g_mac);
        mesh_json_add_elem(sms_msg, SMS_MSG_LEN, g_mac, ESP_MESH_JSON_DEV_MAC_ELEM_LEN);
    }else{
        ESP_DBG("G_MAC: %s\r\n",g_mac);
        g_mac = (char*)mesh_GetMdevMac();
        mesh_json_add_elem(sms_msg, SMS_MSG_LEN, g_mac, ESP_MESH_JSON_DEV_MAC_ELEM_LEN);
    ...
    os_printf("sms msg: %s\r\n",sms_msg);
    espconn_esp_sent(&user_conn, sms_msg, os_strlen(sms_msg),0);
...
user_esp_platform_upgrade_rsp(void *arg)
{
    ...
    if (server->upgrade_flag == true) {
        ...
        espconn_esp_sent(pespconn, pbuf, os_strlen(pbuf),1);
	...
	user_mesh_upgrade_start_func()
{
    ...
    if (!user_mesh_upgrade_build_pkt(buffer, g_mesh_nonce, g_mesh_version, 0, MESH_UPGRADE_SEC_SIZE)) {
        ...
        os_printf("ota build pkt error 02 \r\n");
    ...
    //espconn_esp_sent(g_mesh_esp, buffer, os_strlen(buffer),1);
    espconn_esp_sent(&user_conn, buffer, os_strlen(buffer),1);
                           ...
                           uint32_t offset, uint16_t sect_len)
{
    ...
    if (!user_mesh_upgrade_build_pkt(buffer, nonce, g_mesh_version, offset, sect_len)) {
		os_printf("build packet error in user_mesh_upgrade_continue\r\n");
    ...
    espconn_esp_sent(esp, buffer, os_strlen(buffer),1);
}
...
user_esp_mesh_upgrade(struct espconn *esp, char *pkt, size_t pkt_len, char *base64_bin)
{
    ...
    if(offset==0 && total_len>0){
		...
		if(upgrade_erase_flg == false){
            ...
            user_mesh_upgrade_delay_start(10000);
    ...
    if (offset + sect_len <= total_len) {
        ...
        if (offset + sect_len < total_len) {
			...
			os_printf("after upgrade continue...\r\n");
			return;
        }
...
user_esp_mesh_upgrade_init(struct espconn *esp, int nonce, uint8_t *pkt, uint16_t pkt_len)
{
    ...
    system_upgrade_flag_set(UPGRADE_FLAG_START);
    #endif
	os_printf("debug : send ota resp\r\n");
    user_platform_rpc_set_rsp(esp, nonce, 200);

	
    //user_mesh_upgrade_continue(esp, pkt, pkt_len, 0, MESH_UPGRADE_SEC_SIZE);//debug
...
user_esp_platform_recv_cb(void *arg, char *pusrdata, unsigned short length)
{
    ...
    char *pstr = NULL;
    #if ESP_MESH_SUPPORT
    //char pbuffer[1024 * 2] = {0};//It is supposed that in mesh solution, we will only receive the entire packet
...
user_esp_platform_init_peripheral(void)
{
    ...
    if (rtc_info->reason == REASON_WDT_RST ||
		...
		if(rtc_info->reason == REASON_WDT_RST) 
			light_shadeStart(HINT_PURE_RED,500,4,2,NULL);
	    else if(rtc_info->reason == REASON_EXCEPTION_RST) 
			light_shadeStart(HINT_PURE_GREEN,500,4,2,NULL);
	    else if(rtc_info->reason == REASON_SOFT_WDT_RST) 
			light_shadeStart(HINT_PURE_BLUE,500,4,2,NULL);
...
user_esp_platform_connect_ap_cb()
{
	...
	#if LIGHT_DEVICE
	os_printf("AP CONNECTED CB,SET WHITE\r\n");
