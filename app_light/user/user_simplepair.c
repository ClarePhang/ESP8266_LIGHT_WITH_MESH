...
void ICACHE_FLASH_ATTR sp_DispDeletMacList(uint8*Tag,uint8(* List)[DEV_MAC_LEN])
{
    uint8 i=0,j=0;
    os_printf("%s",Tag);
    for(i=0;i<MAX_BUTTON_NUM;i++){
        os_printf("cnt=%d MAC:",i);
        for(j=0;j<DEV_MAC_LEN;j++){
            os_printf("%02x ",List[i][j]);
        }
        os_printf("\n");
    }
    os_printf("\n");  
...
sp_GetDeletButtonCnt(const char* buff)
{
    ...
    if(NULL==tag_pos){
        os_printf("buff canot find delet button len tag\n");
        return  -1;
    }
    tag_pos+=10;
    if(','==*(tag_pos+1)){
        ...
        if(cnt>MAX_BUTTON_NUM){
            os_printf("will delet button cnt=%d  err!!!\n",cnt);
            return -1;
        }
        else return cnt;
    }
    else{
        os_printf("delet button mac format err 0x%x\n",*(tag_pos+1));
        return -1;
    }
}
